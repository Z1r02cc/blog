...
public class CommonUtils {
    ...
    public static String getClasspath(){
        File path = null;
        try {
            path = new File((ResourceUtils.getURL("classpath:").getPath()).split("target")[0]);
    ...
    public static Boolean verifyCode(HttpSession session, String userInput){
        String circleCaptchaCode = (String) session.getAttribute("circleCaptchaCode");
        System.out.println(circleCaptchaCode);
        System.out.println(!StrUtil.isBlank(userInput) && userInput.equalsIgnoreCase(circleCaptchaCode));
        return !StrUtil.isBlank(userInput) && userInput.equalsIgnoreCase(circleCaptchaCode);
    }
